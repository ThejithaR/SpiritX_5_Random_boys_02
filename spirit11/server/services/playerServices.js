...
export const computePlayerStats = (
  ...
  const playerPoints =
    battingStrikeRate / 5 +
    battingAverage * 0.8 +
    (bowlingStrikeRate === 0 ? 0 : 500 / bowlingStrikeRate) +
    (economyRate === 0 ? 0 : 140 / economyRate);

  const playerValue =
    Math.round(((9 * playerPoints + 100) * 1000) / 50000) * 50000;
...
export const calPlayerPoints = async (team) => {
  try {
    let totalPoints = 0;
    await Promise.all(
      team.map(async (p) => {
        ...
        if (player) {
          ...
          console.warn(`Player with ID ${p._id} not found`);
        }
      })
    );
